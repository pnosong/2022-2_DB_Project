...
<body>
    ...
    <?php
    $sql = "select ProductInformation.ProductUniqueNumber, ProductInformation.title, ProductInformation.content, ProductInformation.price, ProductInformation.photo 
            from ProductInformation join ProductTransactionStatus on ProductInformation.ProductUniqueNumber = ProductTransactionStatus.ProductUniqueNumber 
    ...
    while (($row = oci_fetch_array($stat)) == TRUE) 
    {
        ...
        echo ("<td>
                <form method = POST action = pur_cancel.php>
                    <input type=hidden name=cancel value=$row[0]>
